...
export class InstructionFormatter {
  ...
  formatNamespaceInfo(namespace: NamespaceInfo) {
    ...
    const name =
      namespace.name == 0 ? "" : constants.string[namespace.name - 1];
    let subNamespaceIndex = 0;
    for (let i = 0; i < constants.namespace.length; i++) {
      const comparisonNS = constants.namespace[i];
      if (
        comparisonNS.kind == namespace.kind &&
        comparisonNS.name == namespace.name
      ) {
    ...
    const subNamespaceString =
      subNamespaceIndex > 0 ? `, "${subNamespaceIndex}"` : "";
  ...
  formatMultinameInfo(multiname: MultinameInfo): string {
    const constants = this.abcFile.constant_pool;
    const typeName = MultinameKind[multiname.kind ?? 0];
    switch (multiname.kind) {
      case MultinameKind.QName:
      case MultinameKind.QNameA: {
        const qnameData = multiname.data as MultinameKindQName;
        const name =
          qnameData.name == 0 ? "" : constants.string[qnameData.name - 1];
        const namespace =
          qnameData.ns == 0 ? null : constants.namespace[qnameData.ns - 1];

        return `${typeName}(${
          !!namespace ? this.formatNamespaceInfo(namespace) : '"null"'
        }, \"${name}\")`;
      }
      ...
      case MultinameKind.RTQNameA: {
        ...
        const name =
          rtQNameData.name == 0 ? "" : constants.string[rtQNameData.name - 1];
      ...
      case MultinameKind.MultinameA: {
        ...
        const name =
          multinameData.name == 0
            ? ""
            : constants.string[multinameData.name - 1];
      ...
      case MultinameKind.TypeName: {
        ...
        const baseMultiname =
          typenameData.qname == 0
            ? null
            : constants.multiname[typenameData.qname - 1];
        const params = typenameData.params.map((param) =>
          param == 0 ? null : constants.multiname[param - 1]
        );
        const paramStrings: string[] = params.map((v) =>
          !!v ? this.formatMultinameInfo(v) : "null"
        );
        return `${typeName}(${
          baseMultiname == null
            ? "null"
            : this.formatMultinameInfo(baseMultiname)
        }<${paramStrings.join(",")}>)`;
      }
